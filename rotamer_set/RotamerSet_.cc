...
#include <core/pack/rotamer_set/WaterAnchorInfo.hh>// jag, used to generate water rotamers  yumeng
...
#include <basic/options/option.hh>  // jag, used to generate water rotamers yumeng
	...
	// that this one switches between....
		...
		if ( existing_residue.name() == concrete_residue->name() ) {
			...
			push_back_rotamer( rot );
                ...
                if ( basic::options::option[ basic::options::OptionKeys::hydrate::water_rotamers_cap].user()) {    //yumeng
...
void
RotamerSet_::build_filtered_tp3_water_rotamers( // jag   yumeng
		pose::Pose const & pose,
		scoring::ScoreFunction const & scorefxn,
		task::PackerTask const & task,
		chemical::ResidueTypeCOP concrete_residue,
...
)
{
	...
	// jag, if we construct more rotamers than "water_rotamers_cap" we filter them
...
)
{
	...
	if ( pose.residue(resid()).name() == "TP3" && water_info[ resid() ].enforced()  == false ){
		...
		Size away_rotamers = num_rotamers();  // jag, we build as many away rotamers as rotamers near the protein
...
void
RotamerSet_::filter_water_rotamers(  // jag
		pose::Pose const & pose,
		scoring::ScoreFunction const & scorefxn,
		task::PackerTask const & task,
		graph::GraphCOP packer_neighbor_graph,
	...
	Size ratio(0);
	Size originally_accepted(0);    // Here we filter rotamers first by bump_check  jag
	...
	for ( Size ii=1; ii<= new_rotamers.size(); ++ii ) {
		...
		// debug output
		//std::cout << new_rotamers.size() << "\t bump_check \t " << originally_accepted << "\t" << resid() << "\n ";
		
	if (originally_accepted > water_rotamers_cap )
	...
	for ( Size ii=1; ii<= new_rotamers.size(); ++ii ) {
		if (accepted_rot[ii] == true){
			++accepted;
			//std::cout << "pre_" << resid() << "\t" << new_rotamers[ii]->xyz(1)[0] << "\t";
			...
			if (accepted%accept_every == 0){
				...
				//std::cout << "post_" << resid() << "\t" << filtered_rotamers[ii]->xyz(1)[0] << "\t";
				//std::cout << filtered_rotamers[ii]->xyz(1)[1] << "\t" << filtered_rotamers[ii]->xyz(1)[2] << "\n";
			}
		}
	}
}
...
)
{
	...
	if ( concrete_residue->name() == "TP3" ) { // TIP3 water /////////////////////////////////
		...
		prepare_for_new_residue_type( *concrete_residue );
                ...
                // jag, if we construct more rotamers than "water_rotamers_cap" we filter them      yumeng
                utility::vector1< ResidueOP > filtered_rotamers;
                filter_water_rotamers( pose, scorefxn, task, packer_neighbor_graph, new_rotamers, filtered_rotamers);
