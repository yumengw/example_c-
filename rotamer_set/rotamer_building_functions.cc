...
#include <basic/options/option.hh>      // jag, added to have water rotamers functioning
...
Vector
build_optimal_hyp_H_for_donor(      // jag     yumeng
        Vector const & o1,
...
Vector
build_optimal_hyp_H_for_acceptor(       // jag
        Vector const & o2,
...
)
{
	using namespace scoring::hbonds;
        using namespace basic::options;         // jag
	...
	Real const o12_dis2_cutoff( 25.0 ); // wild guess
        Real const hbond_energy_threshold( option[ hbond_threshold ]() ); // jag, turned into an option
...
void
build_sew_waters_to_donor(      // jag     yumeng
...
//                                      tt << "hbenergies: d angle_step= " << angle_step << " E1= " 
//                                      << ObjexxFCL::format::F(9,3,energy1) << std::endl;
...
//                              tt << "hbenergies: a angle_step= " << angle_step <<
//                              " E2= " << ObjexxFCL::format::F(9,3,energy2) << std::endl;
...
void
build_single_edge_waters(       // jag    yumeng
        conformation::Residue const & rsd,
        Size anchor_atom,
        chemical::ResidueType const & h2o_type,
        conformation::Residue const & tp5,
...
)
{
        ...
        if ( rsd.atom_type( anchor_atom ).is_polar_hydrogen() ){
                build_sew_waters_to_donor( rsd, anchor_atom, h2o_type, tp5, new_rotamers );
        } else{
                build_sew_waters_to_acceptor( rsd, anchor_atom, h2o_type, tp5, new_rotamers );
        }
}
...
)
{
	...
	for ( chemical::AtomIndices::const_iterator
		...
		if ( rsd2.xyz( hatm ).distance_squared( anchor_xyz ) > dis2_cutoff ) continue;
                if ( rsd1.seqpos() == rsd2.seqpos() && anchor_atom == hatm) continue; // jag Both anchors can't be the same atom
	...
	for ( chemical::AtomIndices::const_iterator
		...
		if ( rsd2.xyz( aatm ).distance_squared( anchor_xyz ) > dis2_cutoff ) continue;
                if ( rsd1.seqpos() == rsd2.seqpos() && anchor_atom == aatm) continue; // jag Both anchors can't be the same atom
...
)
{
	...
	for ( graph::Graph::EdgeListConstIter
		...
		//assert( i != j );   // jag commented; There is no reason why both anchor atoms should not be in the same res (glu, asp)
...
/////////////////////////////////////////////////////////////////////////
void
build_anchorless_water_rotamers(      // jag
        Size const seqpos,
        chemical::ResidueType const & h2o_type,
        pose::Pose const & pose,
        utility::vector1< conformation::ResidueOP > & new_rotamers
)
{
        ...
        //tt << "Building anchorless rotamers" << std::endl;

        assert( h2o_type.natoms() == 3 ); // only works for this guy now
        chemical::ResidueTypeSet const & residue_set( h2o_type.residue_type_set() );
        ResidueOP tp5( ResidueFactory::create_residue( residue_set.name_map("TP5") ) );
        ...
        for (Real ix = -SP_RES; ix <= SP_RES; ++ix)     //  Building cubic grid
                for (Real iy = -SP_RES; iy <= SP_RES; ++iy)
                        for (Real iz = -SP_RES; iz <= SP_RES; ++iz){
                                ...
                                // debug output
                                //tt << ix << "\t" << iy << "\t" << iz << std::endl;
                                //tt << (starting_O + ix*step_x + iy*step_y + iz*step_z)[0] << "\t";
                                //tt << (starting_O + ix*step_x + iy*step_y + iz*step_z)[1] << "\t";
                                //tt << (starting_O + ix*step_x + iy*step_y + iz*step_z)[2] << std::endl;
				  ...
				  // Placing oxygen in lattice point
                                ...
                                for ( Real theta_step = 1; theta_step <= ANG_RES; ++theta_step){
                                        ...
                                        for ( Real phi_step = 1; phi_step <= 2*ANG_RES; ++phi_step){
                                                ...
                                                Vector const & betweenH ( (xyz_H1 + xyz_H2).normalize() );
                                                Vector const & perp_1 (betweenH.cross(xyz_H1).normalize() );
                                                Vector const & perp_2 (betweenH.cross(perp_1).normalize() );
                                                ...
                                                new_rotamers.push_back( create_oriented_water_rotamer( h2o_type, xyz1b, xyz_O, xyz2b, "H1", "H2", *tp5 ) );
...
void
build_single_anchor_water_rotamers_independet(  // jag     yumeng
        WaterAnchorInfo const & water_info,
        chemical::ResidueType const & h2o_type,
        pose::Pose const & pose,
        utility::vector1< conformation::ResidueOP > & new_rotamers
)
{
        ...
        Size const anchor_atom( water_info.anchor_atom( pose.residue(anchor_residue).type() ) );

        conformation::ResidueOP tp5( conformation::ResidueFactory::create_residue(
                h2o_type.residue_type_set().name_map("TP5") ) );

        build_single_edge_waters( pose.residue(anchor_residue), anchor_atom, h2o_type, *tp5, new_rotamers);
}
	...
	if ( pose.data().has( core::pose::datacache::CacheableDataType::WATER_PACKING_INFO ) ) {
		WaterPackingInfo const & water_info
			( static_cast< WaterPackingInfo const & >( pose.data().get( core::pose::datacache::CacheableDataType::WATER_PACKING_INFO ) ) );
                ...
                if ( water_info[seqpos_water].anchor_atom() == "NONE" ){        // jag
                        build_anchorless_water_rotamers(seqpos_water, h2o_type, pose, new_rotamers);
                } else {
                        if ( water_info[seqpos_water].rotamer_bonds() == "SINGLE" ){
                                build_single_anchor_water_rotamers_independet( water_info[ seqpos_water], h2o_type, pose, new_rotamers );
                                ...
                                build_moving_O_water_rotamers_independent( water_info[ seqpos_water], h2o_type, task, pose,
                                         packer_neighbor_graph, new_rotamers ); // default behavior 
                        }
                }
...
)
{
	...
	if ( pose.data().has( core::pose::datacache::CacheableDataType::WATER_PACKING_INFO ) ) {
		WaterPackingInfo const & water_info
			( static_cast< WaterPackingInfo const & >( pose.data().get( core::pose::datacache::CacheableDataType::WATER_PACKING_INFO ) ) );
	        // jag, if anchor_atom is NONE, they only build independent rotamers
